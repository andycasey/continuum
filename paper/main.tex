...
In these situations continuum normalization is often implemented as a pre-processing step that is independent of stellar parameters. This leads to results that are biased, or inconsistent at low and high signal-to-noise ratios.
A more justified approach is to forward model spectra with all nuisances simultaneously, but in practice this can be an expensive or non-convex optimization procedure.
...
This model characterizes all possible spectra in many fewer parameters than comparable data-driven models.
The non-negativity constraint ensures basis vectors are strictly additive, which prohibits most unphysical spectra such that we can distinguish normalized spectra from the joint instrument-continuum response.
The joint continuum-instrument response is modelled with a Fourier basis.
%Together this set of bases represents a flexible linear model that can predict spectra of many types of stars, and complex continuum/instrument shapes with hundreds of parameters.
