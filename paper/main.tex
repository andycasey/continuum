...
We found it usually takes a few core-seconds to fit each exposure ($3\times10^6$~pixels; all echelle orders). Most of this time is spent constructing the design matrix, which required (linearly) interpolating the stellar and telluric basis vectors on to the observed wavelengths. The fits to each star are partially shown in Figure~\ref{fig:harps-example}. In all cases the data are shown in black, with the stellar absorption model in \todo{red}, telluric transmission in \todo{color}, and continuum in \todo{color}. In bluer echelle orders, there are no pixels that are true `continuum' pixels (i.e., no absorption). Any continuum normalization method trying to fit these echelle orders would be biased if they did not have a good representation of what stellar spectra look like. Overall, the fits to the data look very good, with reduced $\chi^2$ values from \todo{A to B}.\\
